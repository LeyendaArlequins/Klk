...
function Library:CreateFloatingIcon(funcName, displayName, callback, initialState)
    ...
    local saveKey = "ZL_ICON_" .. funcName .. "_POS"
    ...
    -- Posiciones predefinidas (solo si no hay posición guardada)
    local gridPositions = {
        Aimbot = {x = 720, y = 120},
        SpinBot = {x = 720, y = 160},
        AutoHit = {x = 720, y = 200},
        GrabActivator = {x = 720, y = 240},
        SpeedBoost = {x = 820, y = 280},
        ...
        AutoSteal = {x = 640, y = 120}
    }
    
    -- Usar posición guardada o posición predefinida
    local posX, posY
    if savedPosition then
        posX = savedPosition.X.Offset
        posY = savedPosition.Y.Offset
    else
        local pos = gridPositions[funcName] or {x = 100, y = 100}
        posX = pos.x
        posY = pos.y
    end
    ...
    -- Sistema para guardar posición cuando se arrastra
    local lastPosition = icon.Position
    local isDragging = false
    
    icon.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            isDragging = true
            lastPosition = icon.Position
        end
    end)
    ...
    icon.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            -- Solo guardar si realmente se movió
            if icon.Position ~= lastPosition then
                -- Guardar posición
                if writefile then
                    local posData = {
                        X = {Scale = icon.Position.X.Scale, Offset = icon.Position.X.Offset},
                        Y = {Scale = icon.Position.Y.Scale, Offset = icon.Position.Y.Offset}
                    }
                    
                    local success, error = pcall(function()
                        writefile(saveKey .. ".json", HttpService:JSONEncode(posData))
                    end)
                    
                    if not success then
                        warn("No se pudo guardar posición del icono:", error)
                    end
                end
            end
        end
    ...
    -- También guardar cuando se suelte el mouse (por si acaso)
    UserInputService.InputEnded:Connect(function(input)
    ...
    local iconState = initialState or true
    ...
    -- Click simple para activar/desactivar la función
